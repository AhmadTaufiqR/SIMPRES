...
<body>
                    ...
                    <ul class="navbar-nav" id="navbar-nav">
                        ...
                        <li class="nav-item">
                            <a class="nav-link menu-link" href="/generation">
                                <i class="las la-calendar-alt"></i> <span data-key="t-headmasters">Detail Presensi</span>
                            </a>
                        </li>
                    </ul>
                </div>
                    ...
                    <div class="col-lg-12">
                        <div class="card" id="invoiceList">
                            <div class="card-header border-0">
                                <div class="d-flex align-items-center">
                                    <h5 class="card-title mb-0 flex-grow-1">Detail Presensi</h5>
    ...
    <thead class="text-muted">
        <tr>
            ...
            <th class="sort text-uppercase" data-sort="name">NIP</th>
            <th class="sort text-uppercase" data-sort="name">Jadwal</th>
            <th class="sort text-uppercase" data-sort="username">Status Kehadiran</th>
            <th class="sort text-uppercase" data-sort="username">Bukti Kehadiran</th>
            <th class="sort text-uppercase" data-sort="action">Action</th>
        </tr>
    </thead>
    <tbody>
        @foreach($detailPresences as $detailPresence)
        <tr>
            <td>{{ $detailPresence->id }}</td>
            <td>{{ $detailPresence->teacher ? $detailPresence->teacher->name : 'N/A' }}</td>
            <td>{{ $detailPresence->teacher ? $detailPresence->teacher->nip : 'N/A' }}</td>
            <td>{{ $detailPresence->course ? $detailPresence->course->jadwal : 'N/A' }}</td>
            <td>{{ $detailPresence->attendance }}</td>
            <td>
                @if($detailPresence->documentation)
                    <img src="{{ asset('path/to/your/images/' . $detailPresence->documentation) }}" alt="Bukti Kehadiran" width="100">
                @else
                    N/A
                @endif
            </td>
            <td>
                <button type="button" class="btn btn-primary" onclick="showImage('{{ $detailPresence->documentation }}')">Show</button>
            </td>
        </tr>
        @endforeach
    </tbody>
...
<script>
    function showImage() {
        // Ambil URL gambar dari elemen dengan ID 'bukti_kehadiran'
        var imageUrl = document.getElementById('bukti_kehadiran').src;
        
        // Tampilkan gambar dalam dialog atau lightbox
        // Anda dapat mengganti cara menampilkan gambar sesuai kebutuhan, seperti dengan menggunakan modal Bootstrap
        alert("URL Gambar: " + imageUrl);
    }
</script>
